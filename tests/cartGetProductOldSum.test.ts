...
test('check first 3 plants', () => {
  ...
  const answer = Math.ceil(
    plants.reduce((acc, plant, ind) => {
      if (ind < 3) {
        return plant.price / (1 - plant.sale / 100) + acc;
      }
      return acc;
    }, 0)
  );
  expect(cart.getProductOldSum()).toBe(answer);
});
...
test('check empty', () => {
  expect(cart.getProductOldSum()).toBe(0);
});
